...
- **第二条根位姿路线：网格搜索**（[`retarget/root_grid.py`](src/web2robot/retarget/root_grid.py)）——
  ...
  和上面那条并列，`--root_solver grid|neural` 切换，输出的是**整段一个常量底座位姿**，
...
| `grid`（网格搜索） | **96.7%** | **28.9%** | **0.387** |

（只看 `examples/` 那 5 段是 91.0% → 99.4%。）grid 多出来的可行率**部分是拿穿模换的**：
它的目标函数里只有"手够不够得到"，没有身体，所以最优解倾向于**把底座吸到手边、手臂
折起来**——够得到了，但手臂贴着躯干走。最干净的证据是 `-0RheyDV3a0_48.6_55.3` 这段：
两条路线 IK 都 100%，穿躯帧 neural 是 6/104、grid 是 **95/104** —— 可行率完全相同，
画面天差地别。**所以 `ik_rate` 单独看会把穿模换来的可行率记成进步**，这也是这个工程
反复强调"指标 ≠ 画面"的由来。

现状：⑤的碰撞过滤能兜住其中大部分，但**没有兜干净** —— 上表 28.9% 就是过滤跑完之后
的残留。给 `neural` 调的那套过滤参数直接套在 grid 更贴身的姿态分布上并不合适，
两条路线的过滤参数分开校准是正在做的事，还没定稿。
...
的原样产物。两张图都由 `scripts/dev/make_readme_assets.py` 从实际 run 目录生成，
...
- **主线是④重定向**。这一环现在有两条根位姿路线可切换（生成模型 / 网格搜索），
  它们的取舍已经量清楚了（见上面④那节），但**还没定哪条是默认** ——
  grid 的可行率高、贴身穿模也多，两条路线的碰撞过滤参数正在分开校准。
- **⑤碰撞过滤修得动大部分、没修干净**：残留穿透帧占比 23.8%（neural）/ 28.9%（grid），
  原因是保真项压着推出项 + 迭代预算有限，不是检测漏了。这是当前最明确的一个待改点。
