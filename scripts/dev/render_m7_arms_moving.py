...
p = parser(__doc__)
p.add_argument("--clip", default="examples/-QALmP1nHtM_678.2_682.2",
               help="片段目录，相对上游 retarget/（m7_tool.sh 已 cd 过去）")
...
print(f"wrote {out}")
for f in [5, 25, 45, 60]:
    if f < T: imageio.imwrite(OUT / f"arms_{f}.png", frames[f])
print(f"saved {OUT}/arms_*.png")
