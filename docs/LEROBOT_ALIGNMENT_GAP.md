...
## 6. 要拍板的（都已在 BACKLOG，本节只做索引）

| 编号 | 一句话 | 不定就做不下去的原因 |
|---|---|---|
| B4 | 第一版数据集要不要三路视频 | 决定任务A能不能独立于任务B交付 |
| B5 | 38 维怎么摆、robot_type 叫什么 | 发出去就是别人训练的口径 |
| B9（新） | fps 逐段不一致怎么处理 | `info.json` 只能写一个 fps |
| B10（新） | `pyarrow` 装哪儿 | 不许往共享 env 装 |
| B11（新） | 我们的 mp4 是 mpeg4，参考要求 h264 | 现有产物违反我方约定 §3，但改它要动 upstream patch |
